...
function StartMission()
    if CurrentCops >= Config.RequiredPolice then
        if StartCoolDownRemaining <= 0 then
            StartCoolDown()
            TriggerServerEvent("RL-ThermiteMission:server:SetActive", true)
            TriggerServerEvent("RL-ThermiteMission:SendDiscordLog")
            SendEmail()
        else
...
RegisterNetEvent("RL-ThermiteMission:SpawnTruck")
AddEventHandler("RL-ThermiteMission:SpawnTruck", function()
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if Distance < 60.0 then
                ...
                TriggerServerEvent("RL-ThermiteMission:Server:GuardsSync", -1)
                TriggerEvent("RL-ThermiteMission:SpawnTruck")
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if Distance < 5.0 then
                ...
                TriggerEvent("RL-ThermiteMission:ResetMission")
                break
            end
        end
    end)
...
RegisterNetEvent("RL-ThermiteMission:ResetMission")
AddEventHandler("RL-ThermiteMission:ResetMission", function()
    StartPeds = {}
    TriggerServerEvent("RL-ThermiteMission:server:SetActive", false)
    InBuyerPoint = false
    InMilitaryPoint = false
end)

RegisterNetEvent('RL-ThermiteMission:client:SetActive', function(status)
    isActive = status
end)
...
RegisterNetEvent('RL-ThermiteMission:GuardsSync', function()
    SpawnGuards()
end)
